const ContactsService = {
	getAllContacts(knex, users_id) {
		return knex.select("*").from("contacts").where("users_id", users_id);
	},
	...
	getAllAddresses(knex, contact_id) {
		return knex.select("*").from("addresses").where("contact_id", contact_id);
	},
	...
	deleteAddress(knex, id) {
		return knex.from("addresses").where("id", id).delete();
	},
