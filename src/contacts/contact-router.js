...
contactRouter
	...
	.get(requireAuth, (req, res, next) => {
		...
		if (search === "" || !search) {
			...
			ContactsService.getSearchResults(db_connection, users_id, search)
				.then((contacts) => {
					res.status(200).json(contacts);
				})
				.catch(next);
	...
	.post(requireAuth, bodyParser, (req, res, next) => {
		...
		for (let field of ["fullname", "phone_number", "picture"]) {
			if (!req.body[field]) {
...
contactRouter
	...
	.post(requireAuth, bodyParser, (req, res, next) => {
		...
		for (let field of ["city", "us_state", "street", "zip", "map"]) {
			if (!req.body[field]) {
				logger.error(`${field} is required`);
				return res.status(400).send(`${field} is required`);
			}
	...
	.delete(requireAuth, bodyParser, (req, res, next) => {
		...
		ContactsService.deleteAddress(db_connection, address_id)
			.then((address) => {
				...
				logger.info(
					`Address with id ${address_id} deleted from address collection`
				);
...
contactRouter
	...
	.post(requireAuth, bodyParser, (req, res, next) => {
		...
		for (let field of ["event_type", "address_id"]) {
			if (!req.body[field]) {
				logger.error(`${field} is required`);
				return res.status(400).send(`${field} is required`);
			}
